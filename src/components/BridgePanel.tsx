  ...
  return (
    <section
      className={`rounded-2xl border p-3 sm:p-4 space-y-3 ${
        boardLive
          ? "border-emerald-700/50 bg-gradient-to-b from-emerald-950/40 to-stone-950"
          : "border-amber-700/50 bg-gradient-to-b from-amber-950/30 to-stone-950"
      }`}
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-[11px] text-stone-400 mt-1 leading-relaxed max-w-md">
            {ko
      ...
      {boardLive ? (
        <div className="text-xs font-mono text-emerald-300/90 bg-black/30 rounded-lg px-3 py-2 border border-emerald-900/40">
          {hostIp || "—"} · age{" "}
          {Number.isFinite(staleMs) ? `${(staleMs / 1000).toFixed(0)}s` : "—"} ·{" "}
          {ko ? "소스엔진 입력 = 이 보드 해시" : "source engine input = this board H"}
        </div>
      ) : (
        <div className="text-xs text-amber-100/90 bg-black/30 rounded-lg px-3 py-2 border border-amber-900/40 leading-relaxed">
          {ko
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        ...
        <a
          href={ps1Href}
          className="text-center rounded-xl border border-stone-600 text-stone-200 text-sm py-3 px-3 hover:border-amber-600/50"
        >
      ...
      {!compact && (
        <div className="text-[10px] font-mono text-stone-400 bg-black/40 border border-stone-800 rounded-lg p-2.5 space-y-1 leading-relaxed">
